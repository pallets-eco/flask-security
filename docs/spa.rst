...
Nginx
-----
When serving a SPA from a Nginx webserver the Flask backend, with Flask-Security-Too, will proabably be served via
...
requests directly from javascript. This has a couple of benefits as all the request happen within the same domain you
...
.. note:: The example doesn't include SSL setup to keep it simple and still suitable for a more complex kubernetes setup
    were Nginx is often used as a load balancer and another Nginx with SSL setup runs in front of it.
...
You can then host you javascript app from a S3 bucket, with or without Cloudfront, GH-pages or from any static webserver.
